Game.GuardarPuntosV = function(game){
};
var valor_edad;
...
Game.GuardarPuntosV.prototype ={
	...
	create:function(){
		...
		this.button_guardar.stage='PuntuacionesV';
	...
	Guardar:function(button){
		...
		var i=Puntuacion_violento.length-1;
		var aux1;
		var aux0;
		
		do{
			aux0=Puntuacion_violento[i].split('|')[0]
			aux1=Puntuacion_violento[i].split('|')[1]
			Puntuacion_violento[i]=nuevo_nombre+'|'+ nueva_puntuacion;
			if(i!=Puntuacion_violento.length-1){
				Puntuacion_violento[i+1]=aux0+'|'+aux1;
			}
			i--;
		}while(i>=0 && Puntuacion_violento[i].split('|')[1]<nueva_puntuacion);
